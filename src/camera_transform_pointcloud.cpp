...
PointCloudTransform::PointCloudTransform()
	: nhPrivate("~")
{
    nhPrivate.param<string>("target_frame", target_frame, "camera_link");
	sub = nhPrivate.subscribe("/cloud", 10, &PointCloudTransform::Callback, this);
	pub = nhPrivate.advertise<sensor_msgs::PointCloud2>("/cloud/tf", 10);
}
...
{
    ros::init(argc, argv, "camera_tramsform_pointcloud");
